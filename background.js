...
// Logic to check tab count and signal content scripts
async function updateTabState() {
    const tabs = await chrome.tabs.query({ url: "*://*.youtube.com/*" });
    const isOnlyOne = tabs.length === 1;

    tabs.forEach(tab => {
        chrome.tabs.sendMessage(tab.id, { action: "SET_STATE", enabled: isOnlyOne });
    });
}

// Listen for tab events to keep state synced
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (tab.url?.includes("youtube.com")) updateTabState();
});

chrome.tabs.onRemoved.addListener(updateTabState);